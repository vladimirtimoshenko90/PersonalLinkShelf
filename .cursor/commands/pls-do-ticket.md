...
4. Change only what the ticket requires. Do not steal work from later tickets. Stick to the CRXJS layout (`manifest.config.ts` at repo root, `src/sidepanel`, `src/background`, `src/store`, `src/storage`, `src/types`).
